...
class FarmerShowTest extends TestCase
{
    ...
    public function test_admin_can_view_farmer_with_listings_and_orders(): void
    {
        ...
        $response = $this->withToken($token)->getJson("/api/v1/admin/farmers/{$farmer->id}");

        $response->assertOk()
            ->assertJsonPath('data.listings_count', 1)
            ->assertJsonPath('data.orders_count', 2)
            ->assertJsonPath('data.total_earned', '90000.00')
            ->assertJsonCount(1, 'data.listings')
            ->assertJsonCount(2, 'data.orders')
            ->assertJsonPath('data.listings.0.produce.name', 'Rice');
    }
}
